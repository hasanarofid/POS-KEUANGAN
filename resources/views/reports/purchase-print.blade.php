...
<head>
    ...
    <title>Laporan Pembelian - {{ \App\Models\Setting::get('site_name', 'MR LUX INDONESIA') }}</title>
    <style>
...
<body>
        ...
        <tbody>
            @forelse($purchases as $index => $purchase)
                <tr>
                    ...
                    <td style="font-weight: 600;">{{ $purchase->supplier_name }}</td>
                    <td style="font-size: 10px; color: #777;">{{ $purchase->note ?? '-' }}</td>
                    <td class="text-right" style="font-weight: 600;">Rp {{ number_format($purchase->total, 0, ',', '.') }}</td>
                </tr>
            @empty
                <tr>
    ...
    @if($totalSum > 0)
        <div class="footer-total">
            <span class="total-label">TOTAL AKUMULASI PEMBELIAN:</span>
